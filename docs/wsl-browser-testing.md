...
4. Keyboard, Radial, Sound, Console selection, and the Porvoz server card are absent, and `#keyboard`, `#radial`, or `#sound` in the address bar falls back to Provider. `#capture` falls back to Prefixes, since that identifier was renamed.
5. `/status-overlay.html` is not served.
6. Provider configurations, connection saving, model loading, prefix editing, prefix import and export, and inference-key copy and rotation all work. Editing a provider configuration leaves routing unchanged. Transcription routing and Instruction routing each select a provider, show its own model catalog, and preserve saved model choices when switched. Routing changes apply to every first-party client connected to the server; third-party inference keys remain bound to their profiles.
